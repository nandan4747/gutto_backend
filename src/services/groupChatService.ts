...
export const removeUserFromGroup = async (
  ...
  try {
    ...
    if (!isAdmin && !isSelfLeaving) {
      throw new Error(
        "You can't just kick people out of a group you don't run.",
      );
    }

    // Don't let the admin leave if they are the only ones left (or handle admin transfer)
    if (
      group.admin.equals(targetUserId) &&
      isSelfLeaving &&
      group.members.length > 1
    ) {
      throw new Error(
        "You are the admin. You can't just abandon your post without promoted someone else first!",
      );
    }
...
export const getUserGroups = async (userId: string) => {
  try {
    ...
    throw new Error(
      "Failed to retrieve your groups. Did you get kicked out of all of them?",
    );
  }
};
...
export const selfremoveFromGroup = async (groupId: string, userId: string) => {
  if (
  ...
  if (!group) {
    throw new Error(
      "Group not found. You can't leave a figment of your imagination.",
    );
  }

  // 3. The crucial check: Admins go down with the ship.
  if (group.admin.toString() === userId) {
    throw new Error(
      "You are the admin! You can't just abandon ship. delete the group first.",
    );
  }

  // 4. Verify they are actually in the group before we try to remove them
  if (!group.members.includes(new mongoose.Types.ObjectId(userId))) {
    throw new Error("You aren't even in this group. Having FOMO?");
  }

  // 5. Yeet the user atomically
  const updatedGroup = await Group.findByIdAndUpdate(
    groupId,
    { $pull: { members: userId } },
  ...
  return {
    message: "Successfully escaped the group chat.",
    group: updatedGroup,
  };
};
